...
ui <- fluidPage(
    ...
    sidebarLayout(
        sidebarPanel(
            ...
            checkboxInput("actualData", "Update to latest data", value = FALSE),
            
        ),
        
        # Show a plot of the Covid19 figures
        mainPanel(
            tabsetPanel(type = "tabs", 
                        tabPanel("Instructions", br(), 
                                 ...
                                 "The figures
                                 are updated on a daily basis by the RIVM. This application is initially loading locally stored data however,
                                 for performance reasons. These locally stored data contain Covid19 figures up to June 17, 2021.",
                                 "You can load the latest figures by selecting the checkbox 'Update to latest data'. Note that 
it can take some time (10 - 20 seconds) for the data to be loaded and the plot to be refreshed."
                                 ),
